...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}" data-theme="dark">
    <head>
        ...
        <title>Add Campaign · Accredited Agent · Pillars &amp; Pride of Afrika</title>
        ...
        <div class="min-h-screen bg-[radial-gradient(circle_at_top,_rgba(212,175,55,0.14),_transparent_55%),_#000000]">
            <header class="border-b border-white/5 bg-black/80 backdrop-blur">
                <div class="poa-container flex items-center justify-between py-5">
                    <div class="leading-tight">
                        ...
                        <div class="mt-1 text-[10px] uppercase tracking-[0.26em] text-neutral-400">Add campaign</div>
                    </div>
            ...
            <main class="poa-container py-8">
                <div class="poa-card max-w-3xl p-6 text-sm text-neutral-200">
                    <form action="{{ route('agent.portfolio.store') }}" method="POST" enctype="multipart/form-data" class="space-y-5">
                        @csrf
                        ...
                        <div>
                            ...
                            <textarea name="description" rows="5" class="w-full rounded-md border border-white/10 bg-black/40 px-3 py-2 text-sm text-white focus:border-[rgba(212,175,55,0.8)] focus:outline-none">{{ old('description') }}</textarea>
                            @error('description')
                                <p class="mt-1 text-xs text-red-400">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            <label class="mb-1 block text-[11px] uppercase tracking-[0.28em] text-neutral-400">Tags (e.g. runway, beauty, commercial)</label>
                            <input type="text" name="tags" value="{{ old('tags') }}" class="w-full rounded-md border border-white/10 bg-black/40 px-3 py-2 text-sm text-white focus:border-[rgba(212,175,55,0.8)] focus:outline-none" />
                            @error('tags')
                                <p class="mt-1 text-xs text-red-400">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            <label class="mb-1 block text-[11px] uppercase tracking-[0.28em] text-neutral-400">Upload images</label>
                            <input type="file" name="images[]" multiple class="w-full text-[12px] text-neutral-300" />
                            <p class="mt-1 text-[11px] text-neutral-500">You can upload multiple images (max ~4MB each).</p>
                            @error('images.*')
                                <p class="mt-1 text-xs text-red-400">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="flex items-center justify-end gap-3 pt-2 text-[11px] uppercase tracking-[0.26em]">
                            <button type="submit" class="rounded-full border border-[rgba(212,175,55,0.9)] bg-[rgba(212,175,55,0.15)] px-6 py-2 text-[rgba(245,230,179,0.98)] hover:bg-[rgba(212,175,55,0.28)]">Save campaign</button>
                        </div>
                    </form>
                </div>
            </main>
